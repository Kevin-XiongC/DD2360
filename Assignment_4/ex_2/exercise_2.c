...
const char *mykernel = " \
__kernel \
void SAXPY (__global float *x, __global float *y, __global float *a) { \
  int global_id_x = get_global_id(0); \
  if (global_id_x >= 200000) return; \
  y[global_id_x] += *a * x[global_id_x]; \
} \
";
...
int main(int argc, char **argv) {
  ...
  float x[ARRAY_SIZE], y[ARRAY_SIZE], r_cpu[ARRAY_SIZE], r_gpu[ARRAY_SIZE];
  float *a = (float *)malloc(sizeof(int));
  ...
  *a = 2.2;
  ...
  err = clEnqueueReadBuffer(cmd_queue, y_dev, CL_TRUE, 0, array_size, r_gpu, 0, NULL, NULL); CHK_ERROR(err);
  ...
  for (int i = 0; i < ARRAY_SIZE; i++) {
    float diff = abs(r_cpu[i] - r_gpu[i]);
    if (diff > MARGIN) {
